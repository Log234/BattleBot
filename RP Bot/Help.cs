...
public class InfoModule : ModuleBase<SocketCommandContext>
{
    ...
    public class EventModule : ModuleBase<SocketCommandContext>
    {
        ...
        {
            IDMChannel channel = await Context.Message.Author.GetOrCreateDMChannelAsync();
            await channel.SendMessageAsync("**!event create** creates a new event and sets it active for you, so that following commands will be directed at this event.");
        }
    ...
    {
        IDMChannel channel = await Context.Message.Author.GetOrCreateDMChannelAsync();
        await channel.SendMessageAsync("**!attack <character alias> <target-1 alias> ... <target-n alias>** the character will attempt to attack one or more targets according to the event ruleset.\nThis only works after the event has started.");
    }
    ...
    {
        IDMChannel channel = await Context.Message.Author.GetOrCreateDMChannelAsync();
        await channel.SendMessageAsync("**!heal <character alias> <target-1 alias> ... <target-n alias>** the character will attempt to heal one or more targets according to the event ruleset.\nThis only works after the event has started.");
    }
    ...
    {
        IDMChannel channel = await Context.Message.Author.GetOrCreateDMChannelAsync();
        await channel.SendMessageAsync("**!ward <character alias> <target-1 alias> ... <target-n alias>** the character will attempt to ward one or more targets according to the event ruleset.\nThis only works after the event has started.");
    }
}
}
